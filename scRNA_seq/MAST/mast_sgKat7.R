...
# ============================================================
# MAST Single-cell Differential Expression Analysis
...
meta <- read.csv(file.path(data_dir, "kat7_filtered_metadata.csv"), stringsAsFactors = FALSE)
meta$X <- paste0("X", meta$X)

message("Reading counts matrix ...")
counts <- read.csv(file.path(data_dir, "kat7_filtered_counts_matrix.csv"), header = TRUE, stringsAsFactors = FALSE)
gene_ids <- counts$X
counts <- counts[, -1]
...
# --- Factor levels ---
expected_groups <- c("unenriched", "sgKat7_enriched")
...
gene_info_file <- file.path(data_dir, "MGIBatchReport_20250815_220553.txt")
...
mast_on_block <- function(this_genes) {
  ...
  tryCatch({
    ...
    contrast_name <- "GroupssgKat7_enriched"
...
write.csv(fcHurdle_full, 
          file = file.path(output_dir, "sgKat7_MAST_DE_results.csv"), row.names = FALSE)

# --- Save all R objects ---
save(sca,
     fcHurdle_full, sig_genes_full,
     lib_size_summary,
     file = file.path(output_dir, "Kat7_MAST_analysis_objects_para.RData"),
     compress=TRUE, version=2
)

# --- Save session info ---
writeLines(capture.output(sessionInfo()), file.path(output_dir, "sgKat7_session_info_para.txt"))
