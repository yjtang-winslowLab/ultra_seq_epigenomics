...
meta <- read.csv(file.path(data_dir, "meaf6_filtered_metadata.csv"), stringsAsFactors = FALSE)
...
counts <- read.csv(file.path(data_dir, "meaf6_filtered_counts_matrix.csv"), header = TRUE, stringsAsFactors = FALSE)
gene_ids <- counts$X
counts <- counts[, -1]
...
# --- Factor levels ---
expected_groups <- c("unenriched", "sgMeaf6_enriched")
...
mast_on_block <- function(this_genes) {
  ...
  tryCatch({
    ...
    contrast_name <- "GroupssgMeaf6_enriched"
...
write.csv(fcHurdle_full, 
          file = file.path(output_dir, "sgMeaf6_MAST_DE_results.csv"), row.names = FALSE)

# --- Save all R objects ---
save(sca,
     fcHurdle_full, sig_genes_full,
     lib_size_summary,
     file = file.path(output_dir, "Meaf6_MAST_analysis_objects_para.RData"),
     compress=TRUE, version=2
)

# --- Save session info ---
writeLines(capture.output(sessionInfo()), file.path(output_dir, "sgMeaf6_session_info_para.txt"))
